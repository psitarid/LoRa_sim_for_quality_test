...
from Tx_mode_functions import select_nodes_to_transmit, check_collisions, check_uplink_finished, change_d_c_phases
...
while(sim.num_to_transmit * ToA/node_step <= 2):
    ...
    if(time% node_step == 1):
        ...
        sim.update_metrics_per_node_step(node_step, gateway, ToA, G, S, node_list, nodes_to_retransmit, nodes_transmitting, waiting_for_ack, collision_rate, len_node_list, len_nodes_transmitting, len_waiting_for_ack, len_nodes_to_retransmit, nodes_selected)
    ...
    change_d_c_phases(node_list, nodes_to_retransmit, waiting_for_ack, nodes_transmitting)
    time += 1
plt.ioff()
